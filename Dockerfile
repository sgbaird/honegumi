...
# and `pip install honegumi` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "honegumi", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n honegumi -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "honegumi", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "honegumi", "YOUR_CONSOLE_SCRIPT"]
