...
obj3_name = "doubling_time"  # ASIDE: will probably treat as an outcome with an outcome constraint, since there's usually a target doubling time # noqa: E501
...
def branin3_moo_mt(
    ...
    # dilution_volume: another way to represent this is you can check the number
    # of cells and then choose a dilution_volume based on a desired seeding
    # density wellplate_position: specifies the position in the wellplate (A1 to
    # C4) cell_type_task: specifies the cell type (heart or liver)
    y = float(
...
# Define total for compositional constraint, where base1 + supplement1 +
# supplement2 == total
total = 10.0
...
ax_client.create_experiment(
    parameters=[
        ...
        # dilution_volume: another way to represent this is you can check the
        # number of cells and then choose a dilution_volume based on a desired
        # seeding density
    ...
    # reparameterized compositional constraint, which is a type of sum constraint
    parameter_constraints=[f"base1 + supplement1 <= {total}"],
)

# Add existing data to the AxClient
for i in range(n_train):
    parameterization = X_train.iloc[i].to_dict()
    # remove supplement2, since it's hidden from search space due to composition
    # constraint
...
for i in range(5):  # Reduced from 23 to 5 iterations to run faster
    ...
    for trial_index, parameterization in list(parameterizations.items()):
        base1 = float(parameterization["base1"])  # type: ignore (alternatively use custom safe_float fn) # noqa: E501
