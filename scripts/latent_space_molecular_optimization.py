...
try:
    ...
    print("RDKit not available, using simulated molecular data")
...
def generate_molecular_dataset(n_molecules=100, seed=42):
    ...
    if RDKIT_AVAILABLE:
        ...
        for i in range(n_molecules):
            molecules.append(
                {
                    "molecular_weight": max(50, np.random.normal(300, 100)),
                    "logp": np.random.normal(2, 1.5),
                    "tpsa": max(0, np.random.exponential(60)),
                    "qed": np.random.beta(2, 2),
                }
            )
        return pd.DataFrame(molecules)
...
def main():
    global vae  # Make vae accessible to objective function
